...
import CloudflareIntegrationPanel from './CloudflareIntegrationPanel';
...
export default function AdminSection({ 
  ...
  // Tabs: 'stats', 'anime', 'seasons_episodes', 'users', 'hash_generator', 'backup_restore', 'bulk_thumbnails', 'auto_thumbnail', 'firebase_config', 'cloudflare'
  const [activeTab, setActiveTab] = useState<'stats' | 'anime' | 'seasons_episodes' | 'users' | 'hash_generator' | 'backup_restore' | 'bulk_operations' | 'banner_manager' | 'bulk_thumbnails' | 'auto_thumbnail' | 'auto_skip_setup' | 'auto_setup_new' | 'aniskip_sync' | 'diagnostics' | 'firebase_config' | 'api_generator' | 'announcements' | 'cloudflare'>('stats');
  ...
  const handleBulkImportSeasons = async () => {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        const seasonId = `${selectedSeasonAnimeId}_${numVal}`;
        const seasonData = {
          id: seasonId,
    ...
    <div className="w-full max-w-7xl mx-auto py-8 px-4 md:px-8 bg-[#0b0813] min-h-screen text-zinc-100 text-left">
      ...
      <div className="flex flex-wrap gap-2.5 mb-8 border-b border-zinc-900 pb-3">
        {[
          ...
          { tab: 'cloudflare', label: 'Cloudflare Gateway ☁️', Icon: Cloud, perm: 'manage_seasons_episodes' },
...
function BackupRestorePanel({ refreshData }: BackupRestorePanelProps) {
  ...
  const handleManualSeed = async () => {
    try {
      ...
      const confirmMsg = `Are you sure you want to seed the standard AniMayX default catalog (Anime, Seasons, and Episodes) into the active ${dbLabel}?\n\nThis will write default collections into your database.`;
      if (!window.confirm(confirmMsg)) return;

      setIsSeeding(true);
      await seedAnimeDatabase(true);
      await refreshData();
      alert(`Database successfully seeded to ${dbLabel}! The default catalog is now fully populated.`);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="space-y-8 text-left animate-fade-in pb-16">
      ...
      {/* SECTION: DIRECT SEEDING MODULE */}
      <div className="glass-panel p-6 rounded-2xl border border-zinc-800 bg-zinc-950/40 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="bg-orange-500/5 px-3 py-2 rounded-lg border border-orange-500/10 inline-flex items-center space-x-2 text-xs font-mono font-bold text-orange-400">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Direct Database Seeding</span>
            </div>
            <h3 className="text-lg font-black text-white">Seed / Reset Default AniMayX Catalog</h3>
            <p className="text-xs text-zinc-400 leading-relaxed max-w-3xl">
              If your database is brand new, or you want to restore the official curated AniMayX starter catalog (including featured series, seasons, and working video streams), you can trigger the direct seeding module. This will write standard collections directly into your active Firestore.
            </p>
          </div>
          <button
            type="button"
            ...
            disabled={isSeeding}
            className="px-6 py-3.5 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 disabled:from-zinc-800 disabled:to-zinc-800 disabled:text-zinc-500 text-black font-black text-xs uppercase rounded-xl active:scale-95 transition-all font-mono tracking-widest shrink-0 flex items-center gap-2 shadow-lg cursor-pointer"
          >
            {isSeeding ? (
              <>
                <div className="animate-spin rounded-full h-3.5 w-3.5 border-2 border-t-black border-zinc-700"></div>
