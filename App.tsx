...
export default function App() {
  ...
  const [isFirestoreDatabaseEmpty, setIsFirestoreDatabaseEmpty] = useState<boolean | null>(null);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
      ...
      if (authUser) {
        ...
        if (profileData.isBanned) {
          ...
          // Automatic seeding on login is disabled for safety to prevent database pollution
          console.log("[Auth State Log] Successfully logged in. Automatic database seeding is disabled.");
        }
      } else {
  ...
  useEffect(() => {
    ...
    const unsubscribeAnime = onSnapshot(animeQuery, (snapshot) => {
      ...
      if (animeList.length > 0) {
        setIsFirestoreDatabaseEmpty(false);
        setAllAnime(animeList);
      } else {
        setIsFirestoreDatabaseEmpty(true);
        setAllAnime(defaultAnime); // fallback to in-memory defaults
      }
    }, (err: any) => {
      console.warn("Firestore onSnapshot subscription anime failure/offline:", err);
      // Fallback to in-memory defaults without writing to Firestore
      setAllAnime(prev => prev.length > 0 ? prev : defaultAnime);
    });
  ...
  useEffect(() => {
    ...
    const isMovie = targetAnime?.type === 'Movie';

    let unsubscribeSeasons = () => {};
    let unsubscribeEpisodes = () => {};

    if (isMovie) {
      const movieSeason: Season = {
        id: 'movie_season',
        animeId: selectedAnimeId,
        number: 1,
        name: 'Movie',
        episodeCount: 1,
        createdAt: targetAnime?.createdAt || new Date().toISOString()
      };
      setActiveDetailsSeasons([movieSeason]);
      setActiveDetailsSelectedSeasonId('movie_season');
      setActiveDetailsEpisodes([]);
      setWatchSelectedSeasonId('movie_season');
    } else {
      // It is a Series
      const loadFallbackSeasons = () => {
        // If Firestore already has data, do not show or create fallback seasons!
        if (isFirestoreDatabaseEmpty === false) {
          setActiveDetailsSeasons([]);
          setActiveDetailsSelectedSeasonId('');
          return;
        }

        // If Firestore is completely empty (first install / preview), display defaults in-memory
      ...
      const loadFallbackEpisodes = () => {
        // If Firestore already has data, do not show or create fallback episodes!
        if (isFirestoreDatabaseEmpty === false) {
          setActiveDetailsEpisodes([]);
          return;
        }
      ...
      unsubscribeSeasons = onSnapshot(seasonsQuery, (snapshot) => {
        ...
        console.warn("Firestore seasons subscription failure:", err);
        loadFallbackSeasons();
      });

      const epsQuery = query(collection(db, 'episodes'), where('animeId', '==', selectedAnimeId));
      unsubscribeEpisodes = onSnapshot(epsQuery, (snapshot) => {
        ...
        console.warn("Firestore episodes subscription failure:", err);
        loadFallbackEpisodes();
      });
    }
  ...
  }, [selectedAnimeId, allAnime, isFirestoreDatabaseEmpty]);
